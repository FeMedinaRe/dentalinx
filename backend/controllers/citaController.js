...
const createCita = async (req, res) => {
  try {
    console.log("Hola");
    var { paciente_id, dentista_id, tratamiento_id, fecha, hora, estado } =
      req.body;
    ...
    //TRATAMIENTO
    tratamiento_id = await tratamiento.findOne(
      { nombre: _id_tratamiento },
      { _id: 1 }
    );
...
const updateCita = async (req, res) => {
  try {
    ...
    var query = await Cita.findByIdAndUpdate(
      id,
      {
...
const getCitasxPaciente = async (req, res) => {
  try {
    const { paciente_id } = req.params;
    const citas = await Cita.find({ paciente_id: paciente_id });
